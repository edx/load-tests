...
class Transaction(object):
    ...
    def run(self):
        ...
        #Bunch of regex to extract the response ID
        iterator = re.finditer('\"type\": \"comment\", \"id\": \"[0-9A-Fa-f]*', html)
        urlid = ""
        for items in iterator:
            urlid = items.group(0).replace('"type": "comment", "id": "', "")
        br.form = HTMLForm(URL_ROOT + '/courses/' + orgname + '/' + coursename + '/' + runtime + '/discussion/comments/' + urlid + '/reply', method='POST', enctype='multipart/form-data')
        ...
        self.custom_timers['overall_response_with_comment'] = latency
