...
def main(argv):
    ...
    for opt, arg in opts:
        if opt == '-h':
            ...
            if arg == 't':
                anonymous = "True"
    data = {"body": str(get_paragraphs(1)[0]), "anonymous_to_peers": anonymous, "user_id": userid, "anonymous": "False", "course_id": orgname + "/" + coursename + "/" + runtime, "api_key": key, "splat": [], "captures": [threadid], "thread_id": threadid}
    print hostname + ":" + portnumber + "/api/v1/threads" + threadid + "/comments"
    result = requests.post(hostname + ":" + portnumber + "/api/v1/threads/" + threadid + "/comments", data=data)
    text = result.text
    print text
    fo = open("commentids.txt", "a")
